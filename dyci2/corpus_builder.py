...
class CorpusBuilder(object):
    ### OLD MIDI FROM SOMAX --> TODO: REMOVE
    # midi_exts = ['.mid', '.midi']
    ...
    # JEROME 06/17
    def read_text(self, path, name, keys=["label"], segtype="beats", fixed_tempo=None,
                  beats_per_bar_dur_puls_s=[None, None],
                  external_beat_markers=None):  # , max_num_label = None, segtype="onsets",usebeats=True):
        ...
        for i in range(0, len(dates) - 1):
            ...
            if previous_beat < len(beats) - 1:
                ...
                if current_time != next_time:
                    ...
                    tempo += local_tempo
                    event["time"]["relative"] = [round(current_beat, 4),
                                                 round(((next_time - current_time) / (next_beat_t - current_beat_t)),
                                                       4)]
                else:
                    event["time"]["relative"] = [round(current_beat, 4),
                                                 round(corpus["data"][i]["time"]["relative"][1], 4)]
