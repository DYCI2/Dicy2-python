...
Algorithms introduced in **Nika, "Guiding Human-Computer Music Improvisation: introducing Authoring and Control with Temporal Scenarios", PhD Thesis, UPMC Paris 6, Ircam, 2016** (https://tel.archives-ouvertes.fr/tel-01361835)
and 
**Nika, Chemillier, Assayag, "ImproteK: introducing scenarios into human-computer music improvisation", ACM Computers in Entertainment, Special issue on Musical metacreation Part I, 2017** (https://hal.archives-ouvertes.fr/hal-01380163).
...
def failure_function_and_right_pos_prefixes(pattern, equiv = (lambda x,y : x == y)):
	...
	:param equiv: compararison function given as a lambda function, default: == 
	:type equiv: function
	:return: Failure function built on the pattern (key = index in the pattern (from 0), value = failure_function[index]), and right positions of the prefixes of the pattern in itself (key = index in the pattern (from 0), value = list: lengths of the prefixes of the pattern in itself ending at the corresponding index).
	...
	while p < len(pattern):
		...
		if p > 1:
			...
			if failure_function_pattern[p] >= 1:
				lengths_internal_prefixes_right_pos_in_pattern[p-1]= lengths_internal_prefixes_right_pos_in_pattern[p-1] + lengths_internal_prefixes_right_pos_in_pattern[failure_function_pattern[p]-1]

	return failure_function_pattern, lengths_internal_prefixes_right_pos_in_pattern


def add_shorter_prefixes(longest_prefixes_pattern_left_pos_in_sequence, internal_prefixes_in_pattern, j, i, print_info = 0):
	""" Sub-routine used in :func:`PrefixIndexing.prefix_indexing`.
	
	Given the prefixes of the pattern found **so far** in the sequence at step i,j in :func:`PrefixIndexing.prefix_indexing` (only the longest in the region [j-length_longest_prefix(step i,j),j-1]),
	return **all the prefixes** of the pattern in the sequence at step i,j
	using the **previously computed internal prefixes in the pattern** (:func:`PrefixIndexing.failure_function_and_right_pos_prefixes`).

 
	:param longest_prefixes_pattern_left_pos_in_sequence: prefixes of the pattern found **so far** (only the longest in region [j-length_longest_prefix,j-1]) in the sequence at step i,j of the research in `PrefixIndexing.prefix_indexing` (key = length, value = list of left positions of the prefixes of the pattern of length 'length' in the sequence)
	:type longest_prefixes_pattern_left_pos_in_sequence: dict (int -> list of int)
	:param internal_prefixes_in_pattern: prefixes of the pattern in itself **resulting from a call to** :func:`PrefixIndexing.failure_function_and_right_pos_prefixes` (key = index in the pattern (from 0), value = list: lengths of the prefixes of the pattern in itself ending at the corresponding index)
	...
	:type print_info: int
	:return: **all the prefixes** of the pattern of the sequence at step i,j of the research (:func:`PrefixIndexing.prefix_indexing`) (key = length, value = list: left positions of prefixes of length 'length')
	...
	if print_info:
		print("--Adding shorter prefixes included in the longest prefix found at step (i,j) = ({},{}).".format(i,j))
	...
	for i0 in range(longest_prefix_right_pos_in_pattern):
		if print_info:
				print("-i0={}: Looking for shorter prefixes ending at index {} in the pattern --> index {} in the sequence...".format(i0, longest_prefix_right_pos_in_pattern-i0, longest_prefix_right_pos_in_sequence-i0))
		for length_prefix in internal_prefixes_in_pattern[longest_prefix_right_pos_in_pattern - i0]:
			...
			if not shorter_prefix_left_pos_in_sequence in prefixes_pattern_left_pos_in_sequence[length_prefix]:
				...
				if print_info:
					print("*!* Shorter prefix found moving backward using the failure function. Left position in the sequence = {}, Length = {}.".format(shorter_prefix_left_pos_in_sequence ,length_prefix))				
...
def prefix_indexing(sequence, pattern, **args):
	...
	:type print_info: int
	:return: prefixes of the pattern in the sequence (key = length, value = list of left positions of prefixes of the pattern of length 'length' in the sequence) **and** length of the longest prefix
	...
	failure_function_pattern, lengths_internal_prefixes_right_pos_in_pattern = failure_function_and_right_pos_prefixes(pattern, equiv)
	...
	while j < len(sequence):
		...
		if print_info:
			print("\nCursor in sequence : j={}. Cursor in pattern : i={}.\nj-i={}. Failure[i]={}\nSequence[j]={}, Pattern[i]={} --> MATCH = {}".format(j,i,j-i,failure_function_pattern[i],sequence[j],pattern[i],match))

		if i > -1 and not match:
			if print_info:
				print("NOT MATCHING AND i>-1 -->\n*!* New prefix of P in S: left position in the sequence = {}, length = {}.".format(j-i,i))
			...
			if i > 0:
				...
				prefixes = add_shorter_prefixes(prefixes, lengths_internal_prefixes_right_pos_in_pattern, j, i, print_info)

			while i > -1 and not match:

				if print_info:
					print("i={} > -1 AND NOT MATCHING (pattern[{}]={}  != sequence[{}]={}).\nUsing the failure_function: i <-f[i]={}".format(i,i,pattern[i], j, sequence[j],failure_function_pattern[i]))
		...
		if (j >= len(sequence)) and (i>0):
			if print_info:
				print("End of the sequence reached while recognizing a prefix.\n*!* New prefix of P in S: left position in the sequence = {}, length = {}.".format(j-i,i))
			for k in range(i,0,-1):
				if not k in prefixes.keys():
					prefixes[k] = []
				if not j-i in prefixes[k]: 
...
def filtered_prefix_indexing(sequence, pattern, **args):
	...
	index_prefixes , max_length = prefix_indexing(sequence, pattern, equiv = equiv, print_info = print_info)
