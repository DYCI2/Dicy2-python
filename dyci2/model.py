...
class FactorOracle(Model):
    ...
    **Factor Oracle automaton class.**
    ...
    def from_state_read_label(self, index_state, label, equiv=None, authorize_factor_links=True):
    ...
    def learn_event(self, state, label, equiv=None):
        ...
        self.add_direct_transition(index - 1, label,
    ...
    def similar_contexts(self, index_state, forward_context_length_min=1, equiv=None):
        ...
        similar_contexts = [index for index in self.similar_backward_context(index_state) if
                            self.length_common_forward_context(index_state, index, equiv) >= forward_context_length_min]
    ...
    def continuations_with_label(self, index_state, required_label, forward_context_length_min=1, equiv=None,
...
# 	In the method free_generation, this method is called with authorized_indexes = possible continuations filtered to
#   	satisfy the constraints of taboos and repetitions.
#   In the method simply_guided_generation, this method is called with authorized_indexes = possible continuations
#   **matching the required label** filtered to satisfy the constraints of taboos and repetitions.
...
# 			filtered_continuations = [c for c in filtered_continuations
# 			     if self.history_and_taboos[c] == min([self.history_and_taboos[ch] for ch in filtered_continuations])]
